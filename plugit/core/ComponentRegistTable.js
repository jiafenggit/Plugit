...
class ComponentRegistTable {
  ...
  * regist(componentRegistations = [], plugit) {
    ...
    for (let {Component, description, attributes, operations, settings} of componentRegistations) {
      ...
      if (this.components[id]) throw new PlugitError(`Component [${id}] has registed! If you regist a custom base component extends the super base Component, please override the static attribute 'type'`);
      ...
      for (let attribute of attributes || []) {
        ...
        yield componentRegistry.registAttribute(attribute, plugit);
      }
      for (let operation of operations || []) {
        ...
        yield componentRegistry.registOperation(operation, plugit);
      }
