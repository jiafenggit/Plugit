...
class ComponentRegistry {
  ...
  * registAttribute({name, type, description} = {}) {
    ...
    console.log(`Attribute [${name}] of component [${this.type}/${this.name}] regist success!`);
  }
  ...
  * registSetting({key, dft, regExp = '^[\\s\\S]*$', description} = {}) {
    ...
    if (this.registedSettings.includes(key)) throw new PlugitError(`Setting [${name}] in component [${this.type}/${this.name}] has registed`);
    ...
    console.log(`Setting [${key}] of component [${this.type}/${this.name}] regist success!`);
  }
